...
function voiceBadge(s){const el=$('#voiceState');if(!el)return;const labels={aus:'Sprachsteuerung aus','wartet':`wartet auf „${s.wake_word}"`,'aufnahme':'hört zu…','denkt':'denkt nach…','spricht':'spricht…','aktiv':'aktiv – einfach sprechen','fehler':'Sprachfehler'};el.textContent=labels[s.state]||s.state;el.classList.toggle('on',!!s.running);const b=$('#voiceToggle');if(b)b.textContent=s.running?'🎙️ Stoppen':'🎙️ Sprachsteuerung';if(s.error)el.title=s.error}
...
const _voiceToggle=$('#voiceToggle');if(_voiceToggle)_voiceToggle.onclick=async()=>{const b=$('#voiceToggle');if(b)b.disabled=true;try{const s=voiceRunning?await api('/api/voice/stop',{method:'POST'}):await api('/api/voice/start',{method:'POST'});voiceRunning=s.running;voiceBadge(s);toast(s.running?'Sprachsteuerung gestartet':'Sprachsteuerung gestoppt')}catch(e){toast(e.message)}finally{const b2=$('#voiceToggle');if(b2)b2.disabled=false}};
pollVoice();
